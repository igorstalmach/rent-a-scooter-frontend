...
export function Map(props: Props) {
    function displayScooters(): JSX.Element[] {
        return props.scooters.scooterList.map(scooter => {
            return (
                <Marker position={[scooter.position.lat, scooter.position.len]}>
                    <Popup>
                        {scooter.name}
                    </Popup>
                </Marker>
    ...
    return(
        <div className='container-map'>
            <MapContainer style={{ height: '95%', width: '95%' }} center={[51.110, 17.06]} zoom={15} scrollWheelZoom={true}>
                ...
                {displayScooters()}
                { props.currPos !== undefined ? <SetCenter currPos={props.currPos.position}/> : null}
            </MapContainer>
        </div>
    );
}